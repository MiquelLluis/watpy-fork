    ...
    * h    : Wave strain (complex-valued)
    ...
    def write_to_txt(self, var, path):
        ...
        if var == 'Psi4':
            ...
            data = np.c_[self.time_ret()/M, self.p4.real/M, self.p4.imag/M, M*self.phase_diff1(var),
                     self.amplitude(var)/M, self.phase(var), self.time]
            ...
            data = np.c_[self.time_ret()/M, self.h.real/M, self.h.imag/M, M*self.phase_diff1(var),
                     self.amplitude(var)/M, self.phase(var), self.time]
